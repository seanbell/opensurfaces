...
PREC kmeans(std::vector<std::vector<double> >CX,
	...
	if (npts < nclus) {
		std::cerr << "Less points given then clusters requested" << std::endl;
		exit(EXIT_FAILURE);
	}
	else if (npts == nclus) {
		// copy X to CX
	...
	{
		std::cerr << "Number of cluters is 0" << std::endl;
		exit(EXIT_FAILURE);
	}
	...
	for (unsigned int n=0; n<nclus; n++ )
		CX.push_back(X[order[i]]);
	...
	if (restarts > 0) {
		...
		std::vector<unsigned int> current_assignment; 
		std::vector<std::vector<double> > current_clustering;

		double minsse = sse;

		for (unsigned int n=1; n<restarts; n++) {

			randperm(order,npts);
			for (unsigned int n=0; n<nclus; n++ )
				current_clustering.push_back(X[order[i]]);
			
			// sse = kmeans_run(current_clustering,X,current_assignment,dim,npts,nclus,maxiter);
			
			if (sse < minsse) {
if KMEANS_VERBOSE>1
					 std::cout << "found a better clustering with sse = " << sse << std::endl;
#endif
				// copy assignment and CX
			}
		}

		// pg: is this necessary ? 
		for (unsigned int i=0; i<best_assignment.size(); ++i)
			best_assignment[i].clear();
		best_assignment.clear();

		for (unsigned int i=0; i<best_clustering.size(); ++i)
			best_clustering[i].clear();
		best_clustering.clear();
