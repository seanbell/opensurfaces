...
{
	this->resize(width,height,this->depth(),this->spectrum(),5);
	return (*this);
}

CImg<T>& resize_nearest_neighbour(int width, int height)
{
	this->resize(width,height,this->depth(),this->spectrum(),1);
	return (*this);
}
...
template<typename C>
CImg<T> get_dot_product_channels(const C& col)
{
...
CImg<T>& normalize_per_channel(const T value_min, const T value_max)
{
	cimg_forC(*this,c)
	{
		get_shared_channel(c).normalize(value_min,value_max);
	}
	return (*this);
}

CImg<Tfloat> get_normalize_per_channel(const T value_min, const T value_max) const {
      return CImg<Tfloat>(*this,false).normalize_per_channel((Tfloat)value_min,(Tfloat)value_max);
}

CImg<Tfloat> get_normalmap_from_depthmap() {
	CImg<Tfloat> channel(this->get_channel(0));
	Tfloat f = 1.0;
	if (channel.max() > 256.0) f=65535.0;
	else if (channel.max() > 1.01) f/=255.0;
	channel/=f;

	CImgList<Tfloat> gradients = channel.get_gradient();
	CImg<Tfloat> sol(channel.width(),channel.height(),channel.depth(),3);
	
	sol.draw_image(0,0,0,0,gradients(0));
	sol.draw_image(0,0,0,1,gradients(1));
	cimg_forXYZ(sol,x,y,z)
	{
		sol(x,y,z,2) = std::sqrt(1.0 - sol(x,y,z,0)*sol(x,y,z,0) - sol(x,y,z,1)*sol(x,y,z,1)); 
	}
	sol+=1.0;
	sol*=0.5;
	sol*=f;
