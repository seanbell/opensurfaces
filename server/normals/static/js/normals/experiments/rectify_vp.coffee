...
class NormalUI
  ...
  update: ->
    ...
    for i in [0..(ncells*2)]
      l1 = new Kinetic.Line(
        fill: '#3c77cc', stroke: 'black', strokeWidth: 2,
...
class Controller
  constructor: (args) ->
    ...
    t = @
    $('.vp-choice').on('click', ->
      shape = window.mt_contents[t.idx]
      uvn = shape.photo.uvn_matrices[$(@).attr('data-idx')]
      console.log uvn
      if uvn?
        t.normal_ui.set_uvn(uvn)
    )
